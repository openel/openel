...
HALFLOAT_T acc_ref_x, acc_ref_y, acc_ref_z, omega_ref_x, omega_ref_y, omega_ref_z;
...
//	HalAddObserver(halMotor02,&halObs201a);
//	HalAddObserver(halMotor02,&halObs201b);
	flgObs = 1;
#if 0
...
//  HALSENSOR_T *sen1;
//	sen1 = (HALSENSOR_T)*halSensor01;
//	sen1 = &halSensor01;
	// ループ
	while(1) {
		usleep(500000); /* 0.5s */
		printf("timer %6d , %5d , %6d : \n",event_count1,event_count2,event_count3);
//		printf("%7.3lf %7.3lf %7.3lf %7.3lf %5d(tmSen)\n",simVal1,simVal2,simVal3,simVal4,tmSensor);
//		printf("%7.3lf\n",*sen1->valueList);
#if 0
		for (i=0;i<28;i++) {
			printf("%d:%x\n", i, (int32_t)value_list[i]);
		}
#endif
#if 1
		printf("Temperature: %7.3lf [K]\n", value_list[3]);
		printf("Acceleration [m/s^2] X: %7.3lf Y: %7.3lf Z: %7.3lf\n", value_list[0], value_list[1], value_list[2]);
		printf("Gyroscope [rad/sec]  X: %7.3lf Y: %7.3lf Z: %7.3lf\n", value_list[4], value_list[5], value_list[6]);
		printf("Geomagnetic [uT]     X: %7.3lf Y: %7.3lf Z: %7.3lf\n", value_list[7], value_list[8], value_list[9]);
#endif

		fflush(stdout);
#if 0
		if ( (150 <= event_count1) && (1==flgObs) ) {
			HalRemoveObserver(halMotor02,&halObs201a);
			HalRemoveObserver(halMotor02,&halObs201b);
			flgObs = 0;
		}
#endif
//		if( 300 <= event_count1 ) break;
		if( 100 <= event_count3 ) break;
	}
