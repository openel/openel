...
static char s[256];
static const char strName[] = "DISTANCESENSOR_EV3";
static uint8_t sn_distance;
...
static const HALPROPERTY_T distancesensor_ev3_property = {
	(char *)strName,
	(char **)strFncLst,
...
static HALRETURNCODE_T fncInit(HALCOMPONENT_T *pHalComponent,HAL_ARGUMENT_T *pCmd) {
	...
	if ( ev3_search_sensor( LEGO_EV3_US, &sn_distance, 0 )) {
		printf( "DISTANCE sensor is found.\n" );
	} else {
		printf( "DISTANCE sensor is NOT found.\n" );
	}
...
static HALRETURNCODE_T fncGetProperty(HALCOMPONENT_T *pHalComponent,HAL_ARGUMENT_T *pCmd) {
	pHalComponent->property = (HALPROPERTY_T *)&distancesensor_ev3_property;
	return HAL_OK;
}
...
HAL_FNCTBL_T HalDistanceSensorEV3Tbl = {
		fncInit,
		fncReInit,
		fncFinalize,
		fncAddObserver,
